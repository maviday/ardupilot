...
static bool handle_bidirectional_landing(AP_Mission::Mission_Command& cmd)
{
    ...
    case 2:
        ...
        gcs_send_text_fmt(PSTR("checking for land starting at index %d"), index);
        ...
        while (true) {
            if (!mission.get_next_nav_cmd(index, land_cmd)) {
                ...
                gcs_send_text_fmt(PSTR("found land index %d"), index);
                break;
            } else {
    ...
    case 2:
        ...
        index = cmd.index+1; // start with looking at the command after the DO_LAND_START
        gcs_send_text_fmt(PSTR("Starting with %d"), index);
        while(mission.read_cmd_from_storage(index, temp_cmd) && temp_cmd.id != MAV_CMD_NAV_LAND) {
            if (mission.is_nav_cmd(temp_cmd)) {
                ...
                gcs_send_text_fmt(PSTR("Replaced %d"), index);
            } else {
                gcs_send_text_fmt(PSTR("Skipped  %d"), index);
            }
            index++;
        }
        gcs_send_text_fmt(PSTR("Ending with %d"), index);
        break;
