...
void Sub::init_rc_in()
{
    ...
    for (int i = 0; i < 6; i++) {
        RC_Channels::set_override(i, source, 1500);
    }

    RC_Channels::set_override(6, source, 1500); // camera pan channel
    RC_Channels::set_override(7, source, 1500); // camera tilt channel

    RC_Channel* chan = RC_Channels::rc_channel(8);
    uint16_t min = chan->get_radio_min();
    RC_Channels::set_override(8, source, min); // lights 1 channel

    chan = RC_Channels::rc_channel(9);
    min = chan->get_radio_min();
    RC_Channels::set_override(9, source, min); // lights 2 channel

    RC_Channels::set_override(10, source, 1100); // video switch
#endif
}
