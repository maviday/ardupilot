...
bool ToshibaLED_I2C::hw_init()
{
    ...
    hal.i2c1->ignore_errors(true);

    // enable the led
    bool ret = (hal.i2c1->writeRegister(TOSHIBA_LED_ADDRESS, TOSHIBA_LED_ENABLE, 0x03) == 0);

    // update the red, green and blue values to zero
    uint8_t val[3] = { _led_off, _led_off, _led_off };
    ret &= (hal.i2c1->writeRegisters(TOSHIBA_LED_ADDRESS, TOSHIBA_LED_PWM0, 3, val) == 0);
