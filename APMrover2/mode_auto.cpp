...
void ModeAuto::do_nav_delay(const AP_Mission::Mission_Command& cmd)
{
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "Delaying %u sec",nav_delay_time_max/1000);
}
...
{
    if (millis() - nav_delay_time_start > nav_delay_time_max) {
        nav_delay_time_max = 0;
        return true;
    }

    // if we're a boat, try to loiter. else, stop
